...
const UserInfo = () => {
  ...
  const getflagColorandText = (anomaly, type) => {
    if (anomaly === null) {
      return { color: "#25B09B", text: type?.chinese };
    ...
    if (userProfile) {
      const parsedData = [
        ...
        { title: "加入日期", content: userProfile.createdAt?.slice(0, 10) },
        {
          title: "廠別",
          content: `${
            userProfile.factorySite?.chinese
              ? userProfile.factorySite?.chinese
              : "-"
          } (${
            userProfile.factorySite?.fullAddress
              ? userProfile.factorySite.fullAddress
              : ""
          })`,
        },
        {
          title: "班制",
          content: userProfile.workDayType?.chinese
            ? userProfile.workDayType.chinese
            : "-",
        },
        {
          title: "工時",
          content: `${userProfile.workHourType?.since.slice(
            0,
            -3
          )} ~ ${userProfile.workHourType?.until.slice(0, -3)}`,
        },
        {
          title: "權限",
    ...
    if (userProfile) {
      getData(`user/${userProfile.id}/attendance?p=1&s=5000`).then((result) => {
        ...
        const formattedEvents = data.map((event) => ({
          id: event.id,
          title: getflagColorandText(event.anomaly, event.type).text,
          color: getflagColorandText(event.anomaly).color,
          start: event.date,
          anomaly: event.anomaly ? event.anomaly : null,
          type: event.type,
        }));
      ...
      >
        ...
        <div className="header-user-info sm:block hidden">
          ...
          <div className="user-name pt-1">
            <h5 className="h5">
              {userProfile ? `歡迎！ ${userProfile.nickname}` : "載入中..."}
            </h5>
          </div>
        </div>
      </div>
