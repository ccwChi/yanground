    ...
    if (dataMode === "richMenu") {
      ...
      navigate(
        `attendance_calendar?user=${dataValue || ""}&dep=${
          apiData.content.find((item) => item.id === dataValue)?.department
            .id || ""
        }&mode=clockPunch`
      );
    } else {
      ...
      >
        <form className="flex flex-col gap-2">
          <div className="inline-flex items-center gap-2">
            <InputTitle
              title={"Line 名稱"}
              classnames="whitespace-nowrap min-w-[80px]"
              pb={false}
              required={false}
            />
            <Controller
          ...
          <div className="inline-flex items-center justify-between gap-2">
            <InputTitle
              title={"性別"}
              classnames="whitespace-nowrap pb-1"
              pb={false}
              required={false}
            />
            <Controller
              ...
              >
                {authorityList?.map((auth) => (
                  <MenuItem key={auth.id} value={auth.id}>
                    <Checkbox
                      checked={watch("permissions").indexOf(auth.id) > -1}
                    />
                    {auth.name}
                  </MenuItem>
                ))}
